...
if SOURCE == 'FLICKR':
    if MODE == 'SUNSET':
        ...
        # OUTPUT store path
        COSINE_SIMILARITY_STORE_PATH = Path("./Semantic_analysis/20210202_FLICKR_SUNSET_random_country_tf_idf.csv") # CHANGE HERE IF NECESSARY
        # STORE PATH FOR INTERMEDIATE PRODUCT - CALCULATED TERMS PER COUNTRY FLICKR POSTS (country_term_dict)
        COUNTRY_TERM_DICT_STORE_PATH = Path("./Semantic_analysis/20210202_FLICKR_SUNSET_country_term_dict.pickle")

    elif MODE == 'SUNRISE':
        # actual sunset/sunrise data. columns: userday, userday_terms, userday_season_id (Multiple/Ambiguous 0, Northern spring 1, Northern summer 2, Northern fall 3, Northern winter 4, Southern spring -1, Southern summer -2, Southern fall -3, Southern winter -4
        DATA_PATH = Path("./Semantic_analysis/Flickr_userday_terms_raw/flickr_sunrise_terms_geotagged_grouped.csv")
        # OUTPUT store path
        COSINE_SIMILARITY_STORE_PATH = Path("./Semantic_analysis/20210202_FLICKR_SUNRISE_country_tf_idf.csv")
        # STORE PATH FOR INTERMEDIATE PRODUCT - CALCULATED TERMS PER COUNTRY FLICKR POSTS (country_term_dict)
    ...
    if MODE == 'SUNSET':
        ...
        # OUTPUT store path
        COSINE_SIMILARITY_STORE_PATH = Path("./Semantic_analysis/20210202_INSTAGRAM_SUNSET_random_country_tf_idf.csv")  # CHANGE HERE IF NECESSARY
        # STORE PATH FOR INTERMEDIATE PRODUCT - CALCULATED TERMS PER COUNTRY FLICKR POSTS (country_term_dict)
        COUNTRY_TERM_DICT_STORE_PATH = Path("./Semantic_analysis/20210202_INSTAGRAM_SUNRSET_country_term_dict.pickle")
        ...
        # OUTPUT store path
        COSINE_SIMILARITY_STORE_PATH = Path("./Semantic_analysis/20210202_INSTAGRAM_SUNRISE_country_tf_idf.csv")
        # STORE PATH FOR INTERMEDIATE PRODUCT - CALCULATED TERMS PER COUNTRY FLICKR POSTS (country_term_dict)
        COUNTRY_TERM_DICT_STORE_PATH = Path("./Semantic_analysis/20210202_INSTAGRAM_SUNRISE_country_term_dict.pickle")
...
def calc_term_vector(country_term_dict, corpus_vocabulary_set, corpus_vocabulary_set_len, country_codes, tracker=1):
    ...
    for index, country_code in enumerate(country_codes, 1):
        ...
        for index2, (term, frequency) in enumerate(country_terms_counter.items()):
            ...
            vocabulary_termindex = corpus_vocabulary_set.index(term) # its actually not a set, but a list of unique items
...
if __name__ == '__main__':
    ...
    # no multiprocessing
        ...
        country_vector_dict = calc_term_vector(country_term_dict, corpus_vocabulary_set, corpus_vocabulary_set_len, country_codes)
